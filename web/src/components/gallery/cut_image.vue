<template>
  <div class="cut-image">
    <el-dialog :title="title" :visible.sync="show" size="small">
      <div class="main">
        <div class="input">
          <button class="btn" type="file">选择图片
            <input class="file" type="file" @change="changeEvent"/>
          </button>
        </div>
        <div class="copper-box">
          <img style="max-width: 100%;" :src="imgUrl" id="images" ref="images"/>
        </div>
...
console.log(cropper);
  export default {
    props: ['show', 'loading', 'ratio', 'title'],
    components: {
      'load-img': loadImg
    },
    data () {
    ...
    methods: {
      changeEvent (e) {
        this.imgUrl = this.getFileUrl(e.target);
        setTimeout(() => {
          this.imgObj = $(this.$refs.images).cropper({
            aspectRatio: this.ratio,
            preview: this.$refs.view,
            movable: false,
            zoomable: false,
            rotatable: false,
            scalable: false
